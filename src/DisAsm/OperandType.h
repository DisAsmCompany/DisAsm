...
typedef enum OperandType_t
{
	...
	E = 0x0010 | MaskModRM, /* instruction has ModR/M byte, general-purpose register operand is defined in R/M field */
	G = 0x0020 | MaskModRM, /* instruction has ModR/M byte, general-purpose register operand is defined in Reg field */
	I = 0x0030, /* immediate data */
	J = 0x0040, /* relative offset */
	M = 0x0050 | MaskModRM, /* instruction has ModR/M byte, but it may refer to memory only */
	O = 0x0060, /* instruction has no ModR/M byte, offset of operand is coded as word or double-word depending on operand-size attribute */
	X = 0x0070, /* Memory addressed by the DS:rSI register pair */
	Y = 0x0090, /* Memory addressed by the ES:rDI register pair */
	F = 0x00A0, /* FLAGS/EFLAGS/RFLAGS */
	...
	U = 0x0100 | MaskModRM, /* instruction has ModR/M byte, 128-bit XMM or 256-bit YMM operand is defined in R/M field */
	V = 0x0110 | MaskModRM, /* instruction has ModR/M byte, 128-bit XMM or 256-bit YMM operand is defined in Reg field */
	W = 0x0120 | MaskModRM, /* instruction has ModR/M byte, 128-bit XMM or 256-bit YMM operand is defined in R/M field */
	P = 0x0130 | MaskModRM, /* instruction has ModR/M byte, MMX operand is defined in Reg field */
	Q = 0x0140 | MaskModRM, /* instruction has ModR/M byte, MMX operand is defined in R/M field */
	A = 0x0150, /* instruction has no ModR/M byte, direct address */
